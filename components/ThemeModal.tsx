        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-gray-800 p-6 rounded-lg max-w-2xl w-full max-h-[85svh] overflow-y-auto border border-gray-700 shadow-xl custom-scrollbar glass-panel" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-yellow-300">배경 테마 변경</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {themes.map((theme) => (
                        <div
                            key={theme.type}
                            className={`relative p-4 rounded-lg border-2 transition-all cursor-pointer hover:scale-[1.02] ${currentTheme === theme.type ? 'border-green-500 bg-green-500/15' : 'border-white/20 bg-white/10 hover:border-white/40'}`}
                            onClick={() => {
